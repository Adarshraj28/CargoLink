    ...
    suspend fun getUserRole(): String? {
        ...
        return try {
            ...
            if (document.exists()) {
                ...
                android.util.Log.d("CargoLink", "Fetched role for $email: $role")
                role ?: "NoRoleField"
            } else {
                android.util.Log.w("CargoLink", "User document missing for $email")
                "ProfileMissing"
            }
        } catch (e: Exception) {
            android.util.Log.e("CargoLink", "Error fetching user role: ${e.message}")
            "Error: ${e.message}"
        }
    }
